...
export default function LeaderboardPlayerDetailPage() {
    ...
    useEffect(() => {
        ...
        fetch(`/api/leaderboard/players/${userId}?${paramsQuery.toString()}`)
            .then((response) => response.json())
            .then((payload) => {
                if (!payload?.success) {
                    setError(t.leaderboard.detail.errors.loadFailed);
                    setSummary(null);
                    return;
                }
    ...
    }, [seasonId, gameId, t.leaderboard.detail.errors.loadFailed, userId]);
    ...
    useEffect(() => {
        ...
        fetch(`/api/leaderboard/players/${userId}/history?${paramsQuery.toString()}`)
            .then((response) => response.json())
            .then((payload) => {
                if (!payload?.success) {
                    setHistory([]);
    ...
    }, [historyPage, seasonId, gameId, userId]);

    useEffect(() => {
        if (!userId || !gameId) return;
        setCompareLoading(true);

        fetch(`/api/leaderboard/players/${userId}/seasons?gameId=${gameId}`)
            .then((response) => response.json())
            .then((payload) => {
                if (!payload?.success) {
                    setSeasonCompare([]);
    ...
    const formatDelta = (value: number | null) => {
        if (value === null) return t.leaderboard.detail.snapshotEmpty;
        return value >= 0 ? `+${value.toFixed(2)}` : value.toFixed(2);
    };

    const streakLabel = trendSnapshot.streak
        ? `${trendSnapshot.streak.type === "W"
            ? t.leaderboard.detail.streakWinShort
            : t.leaderboard.detail.streakLoseShort}${trendSnapshot.streak.count}`
        : t.leaderboard.detail.snapshotEmpty;
    ...
    const displayName = summary?.ign
        || summary?.neighbors?.find((item) => item.user.id === userId)?.user.ign
        || summary?.neighbors?.find((item) => item.user.id === userId)?.user.username
        || summary?.neighbors?.find((item) => item.user.id === userId)?.user.fullName
            ...
            <section className="border-b border-base-300 pt-28">
                <div className="mx-auto max-w-[1200px] px-4 pb-10 sm:px-6 lg:px-8">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <p className="text-sm font-bold uppercase tracking-[0.34em] text-primary">{t.leaderboard.detail.badge}</p>
                            <h1 className="mt-3 text-2xl font-black sm:text-3xl">{t.leaderboard.detail.title}</h1>
                            <p className="mt-2 text-sm text-base-content/60">{t.leaderboard.detail.subtitle}</p>
                        </div>
                        <Link href="/leaderboard" className="btn btn-outline btn-sm rounded-box">
                            {t.leaderboard.detail.back}
                        </Link>
                    </div>
                </div>
            </section>
                    ...
                    <>
                        <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                            <div className="rounded-2xl border border-base-300 bg-base-100 p-5">
                                <div className="text-sm font-semibold text-base-content/60">{t.leaderboard.detail.playerLabel}</div>
                                <div className="mt-1 text-2xl font-black text-base-content">{displayName}</div>
                                <div className="mt-1 text-xs text-base-content/50">{summary.game?.name ?? t.leaderboard.detail.gameFallback}</div>
                                <div className="mt-3 flex flex-wrap gap-3 text-sm text-base-content/70">
                                    <div>{t.leaderboard.detail.rankLabel(summary.rank)}</div>
                                    <div>{t.leaderboard.detail.eloLabel(summary.eloRating.toFixed(2))}</div>
                                    <div>{t.leaderboard.detail.tierLabel(summary.rankTier)}</div>
                                </div>
                                <div className="mt-4 flex flex-wrap gap-3 text-sm text-base-content/60">
                                    <div>{t.leaderboard.detail.recordLabel(summary.wins, summary.losses)}</div>
                                    <div>{t.leaderboard.detail.winRateLabel((summary.winRate * 100).toFixed(1))}</div>
                                </div>
                            </div>
                            <div className="rounded-2xl border border-base-300 bg-base-100 p-5">
                                <div className="text-sm font-semibold text-base-content/60">{t.leaderboard.detail.historyTitle}</div>
                                <div className="mt-3 grid gap-3 sm:grid-cols-3">
                                    <div className="rounded-box border border-base-300 bg-base-200/40 px-3 py-2">
                                        <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-base-content/60">
                                            {t.leaderboard.detail.snapshotLastMatch}
                                        </div>
                                    ...
                                    <div className="rounded-box border border-base-300 bg-base-200/40 px-3 py-2">
                                        <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-base-content/60">
                                            {t.leaderboard.detail.snapshotLast7Days}
                                        </div>
                                    ...
                                    <div className="rounded-box border border-base-300 bg-base-200/40 px-3 py-2">
                                        <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-base-content/60">
                                            {t.leaderboard.detail.snapshotStreak}
                                        </div>
                                ...
                                <div className="mt-3">
                                    <EloHistoryChart values={chartValues} emptyLabel={t.leaderboard.detail.historyEmptyChart} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-6 rounded-2xl border border-base-300 bg-base-100 p-5">
                            <div className="mb-4 flex items-center justify-between">
                                <div className="text-sm font-semibold text-base-content/60">{t.leaderboard.detail.auditTitle}</div>
                                <div className="text-xs text-base-content/50">
                                    {t.leaderboard.detail.auditCount(history.length, historyTotal)}
                                </div>
                            </div>
                            ...
                            {historyLoading ? (
                                ...
                                <div className="rounded-box border border-dashed border-base-300 bg-base-200/40 px-4 py-6 text-center text-sm text-base-content/60">
                                    {t.leaderboard.detail.historyEmpty}
                                </div>
                            ) : (
                                    ...
                                    <table className="table">
                                        <thead>
                                            <tr className="text-xs uppercase tracking-widest text-base-content/60">
                                                <th>{t.leaderboard.detail.auditColumns.date}</th>
                                                <th>{t.leaderboard.detail.auditColumns.change}</th>
                                                <th>{t.leaderboard.detail.auditColumns.elo}</th>
                                                <th>{t.leaderboard.detail.auditColumns.match}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {history.map((entry) => {
                            ...
                            {historyTotal > HISTORY_LIMIT ? (
                                <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                    <div className="text-xs text-base-content/60">
                                        {t.leaderboard.detail.historyPageLabel(historyPage, totalHistoryPages)}
                                    </div>
                                    <div className="join">
                                        <button
                                            type="button"
                        ...
                        <div className="mt-6 rounded-2xl border border-base-300 bg-base-100 p-5">
                            <div className="mb-4 flex items-center justify-between">
                                <div className="text-sm font-semibold text-base-content/60">{t.leaderboard.detail.seasonCompareTitle}</div>
                            </div>
                            {compareLoading ? (
                                <div className="space-y-2">
                                    {[1, 2].map((item) => (
                                ...
                                <div className="rounded-box border border-dashed border-base-300 bg-base-200/40 px-4 py-6 text-center text-sm text-base-content/60">
                                    {t.leaderboard.detail.seasonCompareEmpty}
                                </div>
                            ) : (
                                <div className="overflow-hidden rounded-2xl border border-base-300">
                                    <table className="table">
                                        <thead>
                                            <tr className="text-xs uppercase tracking-widest text-base-content/60">
                                                <th>{t.leaderboard.detail.seasonColumns.season}</th>
                                                <th>{t.leaderboard.detail.seasonColumns.elo}</th>
                                                <th>{t.leaderboard.detail.seasonColumns.record}</th>
                                                <th>{t.leaderboard.detail.seasonColumns.winRate}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
