...
export async function GET(request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
    try {
        const rateLimitResponse = enforceLeaderboardRateLimit(request, "players:history");
        ...
        const [entries, total] = await Promise.all([
            prisma.leaderboardHistory.findMany({
                where: { userId, seasonId, gameId },
                orderBy: { createdAt: "desc" },
                skip,
                take: limit,
            }),
            prisma.leaderboardHistory.count({ where: { userId, seasonId, gameId } }),
        ]);
        ...
        console.error("[Leaderboard Player History]", error);
