...
import { teamMemberPromoteSchema } from "@/lib/validators";
...
export async function POST(request: Request) {
    try {
        ...
        const input = await parseJsonBody(request, teamMemberPromoteSchema);
        const membership = await teamService.promoteMember(user.id, input);
