...
function getStatusTone(status: string) {
    ...
    return "border-slate-300 bg-slate-100 text-slate-600 dark:border-white/10 dark:bg-white/10 dark:text-white/50";
}
...
export default async function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <main className="min-h-screen bg-[linear-gradient(180deg,_#fffdf8_0%,_#f8fafc_100%)] text-slate-950 dark:bg-[#0f0f0f] dark:text-white">
            <Navbar />
            <section className="border-b border-slate-200 bg-[radial-gradient(circle_at_top,rgba(255,201,22,0.18),transparent_25%),linear-gradient(180deg,#fffdf7,#f5f7fb)] pt-28 dark:border-white/10 dark:bg-[radial-gradient(circle_at_top,rgba(255,201,22,0.16),transparent_25%),linear-gradient(180deg,#161616,#0f0f0f)]">
                <div className="mx-auto max-w-7xl px-4 pb-14 sm:px-6 lg:px-8">
                    ...
                    <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
                        <div className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_18px_55px_rgba(15,23,42,0.09)] dark:border-white/10 dark:bg-[#171717] dark:shadow-none">
                            <div className="relative min-h-[320px] border-b border-slate-200 bg-gradient-to-br from-cyan-200 via-white to-amber-50 dark:border-white/10 dark:from-cyan-950 dark:via-zinc-900 dark:to-black">
                                ...
                                <div className="relative flex min-h-[320px] flex-col justify-end p-8">
                                    <div className="mb-4 flex flex-wrap items-center gap-3">
                                        ...
                                        <span className="rounded-full border border-slate-300 bg-white/80 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.25em] text-slate-600 dark:border-white/15 dark:bg-black/25 dark:text-white/70">
                                            {tournament.format}
                                        </span>
                                        <span className="rounded-full border border-slate-300 bg-white/80 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.25em] text-slate-600 dark:border-white/15 dark:bg-black/25 dark:text-white/70">
                        ...
                        <div className="space-y-6">
                            <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_55px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-[#171717] dark:shadow-none">
                                <div className="mb-4 text-sm font-bold uppercase tracking-[0.28em] text-amber-600 dark:text-[#FFC916]">Registration</div>
                                <h2 className="mb-2 text-2xl font-black text-slate-950 dark:text-white">Aksi Tournament</h2>
                            ...
                            <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_55px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-[#171717] dark:shadow-none">
                                <div className="mb-4 text-sm font-bold uppercase tracking-[0.28em] text-amber-600 dark:text-[#FFC916]">Participant Lobby</div>
                                {tournament.participants.length === 0 ? (
                                    <p className="text-sm text-slate-600 dark:text-white/55">Belum ada peserta terdaftar. Jadilah yang pertama masuk bracket ini.</p>
                                ) : (
                                    <div className="space-y-3">
                                        {tournament.participants.map((participant, index) => (
                                            <div key={participant.id} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-white/10 dark:bg-white/[0.03]">
