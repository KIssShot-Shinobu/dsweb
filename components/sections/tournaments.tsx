            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.35 }} transition={{ duration: 0.45 }} className="mb-10 flex flex-col gap-5 sm:mb-12 sm:gap-6 md:flex-row md:items-end md:justify-between">
                    <div className="max-w-2xl">
                        <p className="mb-3 text-sm font-bold uppercase tracking-[0.32em] text-amber-600 dark:text-[#FFC916]">Tournament Publik</p>
                        <h2 className="mb-3 text-3xl font-black text-slate-950 sm:text-4xl dark:text-white">Event terbaru, tampil rapi dalam satu grid.</h2>
                        <p className="text-slate-600 dark:text-[#E6E6E6]/60">Lihat event yang sedang dibuka, cek hadiah, lalu lanjut ke detail saat Anda siap masuk bracket.</p>
                    </div>
                ...
                {tournaments.length === 0 ? (
                    <div className="rounded-[28px] border border-slate-200 bg-white px-6 py-14 text-center shadow-[0_18px_55px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] dark:shadow-[0_18px_55px_rgba(0,0,0,0.18)] sm:py-16">
                        ...
                        <p className="text-base text-slate-500 sm:text-lg dark:text-white/55">Belum ada event aktif. Begitu bracket baru dibuka, tampilannya akan muncul di sini.</p>
                    </div>
                ) : (
