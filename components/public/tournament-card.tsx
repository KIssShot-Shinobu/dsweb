...
function getStatusLabel(status: string) { if (status === "OPEN") return "Registrasi Dibuka"; if (status === "ONGOING") return "Sedang Berlangsung"; if (status === "COMPLETED") return "Selesai"; if (status === "CANCELLED") return "Dibatalkan"; return status; }
    ...
    return (
        <article className="group overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_55px_rgba(15,23,42,0.09)] transition-all hover:-translate-y-1 hover:border-[#FFC916]/40 active:scale-[0.99] dark:border-white/10 dark:bg-[#11161d] dark:shadow-[0_20px_60px_rgba(0,0,0,0.32)]">
            ...
            <div className="space-y-4 p-4 sm:space-y-5 sm:p-5">
                ...
                {!compact ? <div className="flex flex-col gap-1 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between dark:text-white/60"><span>{tournament.participantCount ?? 0} peserta terdaftar</span><span>{tournament.entryFee && tournament.entryFee > 0 ? formatPrize(tournament.entryFee) : "Tanpa biaya pendaftaran"}</span></div> : null}
                <p className={`text-sm leading-6 text-slate-600 dark:text-white/62 ${compact ? "line-clamp-2" : "line-clamp-3"}`}>{tournament.description?.trim() || "Turnamen komunitas Duel Standby dengan alur yang tertata, informasi yang transparan, dan atmosfer kompetitif yang sehat."}</p>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <Link href={`/tournaments/${tournament.id}`} className="inline-flex flex-1 items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 transition-all hover:border-slate-300 hover:bg-white active:scale-[0.98] dark:border-white/12 dark:bg-[#161b23] dark:text-white dark:hover:border-white/25 dark:hover:bg-[#1b2230]">Lihat Detail</Link>
                    {!compact && tournament.status === "OPEN" ? <Link href={`/tournaments/${tournament.id}`} className="inline-flex flex-1 items-center justify-center rounded-2xl bg-[#FFC916] px-4 py-3 text-sm font-black text-[#111111] transition-all hover:bg-[#ffd84c] active:scale-[0.98]">Daftar Sekarang</Link> : null}
                </div>
            </div>
        </article>
    );
