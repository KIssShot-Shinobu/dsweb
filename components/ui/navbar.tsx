...
export function Navbar() {
    ...
    return (
        <nav className="fixed left-0 right-0 top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-md dark:border-white/10 dark:bg-[#2E2E2E]/90">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="flex h-16 items-center justify-between sm:h-[4.5rem]">
                    <div className="flex items-center gap-2"><Swords className="h-8 w-8 text-[#FFC916]" /><span className="bg-gradient-to-r from-[#FFC916] to-[#FFC000] bg-clip-text text-lg font-bold text-transparent sm:text-xl">DuelStandby</span></div>
                    <div className="hidden md:block">
                        <div className="ml-10 flex items-center space-x-3 lg:space-x-4">
                            {navLinks.map((link) => <Link key={link.name} href={link.href} className="rounded-md px-3 py-2 text-sm font-medium text-slate-600 transition-colors hover:text-[#FFC916] dark:text-gray-300">{link.name}</Link>)}
                            <button onClick={toggleTheme} className="rounded-md border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 transition-colors hover:text-[#FFC916] active:scale-[0.98] dark:border-white/15 dark:text-gray-300">{theme === "dark" ? "Light" : "Dark"}</button>
                            {!me ? (<><Link href="/login" className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition-all hover:border-[#FFC916]/40 hover:text-[#FFC916] active:scale-[0.98] dark:border-white/15 dark:text-gray-300">Sign In</Link><Link href="/register" className="rounded-lg bg-gradient-to-r from-[#FFC916] to-[#FFC000] px-4 py-2 text-sm font-semibold text-[#2E2E2E] transition-all hover:shadow-[0_0_20px_rgba(255,201,22,0.3)] active:scale-[0.98]">Sign Up</Link></>) : (
                                <div className="relative" ref={menuRef}>
                                    ...
                                    <AnimatePresence>{menuOpen && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="absolute right-0 mt-2 w-44 rounded-xl border border-slate-200 bg-white p-2 shadow-xl dark:border-white/10 dark:bg-[#1F1F1F]"><Link href={isAdmin ? "/dashboard" : "/dashboard/profile"} onClick={() => setMenuOpen(false)} className="block rounded-lg px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 dark:text-gray-200 dark:hover:bg-white/10">{isAdmin ? "Dashboard" : "Profile"}</Link>{!me.emailVerified && <Link href="/dashboard/settings" onClick={() => setMenuOpen(false)} className="block rounded-lg px-3 py-2 text-sm text-amber-600 hover:bg-amber-50 dark:text-amber-300 dark:hover:bg-amber-500/10">Verifikasi Email</Link>}<button onClick={handleLogout} className="w-full rounded-lg px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-500/10">Logout</button></motion.div>}</AnimatePresence>
                                </div>
                            )}
                        </div>
                    </div>
                    <div className="md:hidden"><button onClick={() => setIsOpen(!isOpen)} className="inline-flex items-center justify-center rounded-xl p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-[#FFC916] focus:outline-none dark:text-gray-400 dark:hover:bg-[#3A3A3A]">{isOpen ? <X className="block h-6 w-6" /> : <Menu className="block h-6 w-6" />}</button></div>
                </div>
            </div>
            <AnimatePresence>{isOpen && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden border-b border-slate-200 bg-white/95 dark:border-white/10 dark:bg-[#2E2E2E]/95 md:hidden"><div className="space-y-2 px-3 pb-4 pt-3">{navLinks.map((link) => <Link key={link.name} href={link.href} onClick={() => setIsOpen(false)} className="block rounded-xl px-4 py-3 text-base font-medium text-slate-600 transition-colors hover:bg-slate-100 hover:text-[#FFC916] dark:text-gray-300 dark:hover:bg-white/5">{link.name}</Link>)}<button onClick={toggleTheme} className="block w-full rounded-xl border border-slate-200 px-4 py-3 text-left text-base font-medium text-slate-600 transition-colors hover:text-[#FFC916] active:scale-[0.99] dark:border-white/15 dark:text-gray-300">{theme === "dark" ? "Light Mode" : "Dark Mode"}</button>{!me ? <div className="grid grid-cols-1 gap-2 pt-1"><Link href="/login" onClick={() => setIsOpen(false)} className="block rounded-xl border border-slate-200 px-4 py-3 text-center text-base font-medium text-slate-600 transition-colors hover:text-[#FFC916] dark:border-white/15 dark:text-gray-300">Sign In</Link><Link href="/register" onClick={() => setIsOpen(false)} className="block rounded-xl bg-gradient-to-r from-[#FFC916] to-[#FFC000] px-4 py-3 text-center text-base font-semibold text-[#2E2E2E]">Sign Up</Link></div> : <div className="grid grid-cols-1 gap-2 pt-1"><Link href={isAdmin ? "/dashboard" : "/dashboard/profile"} onClick={() => setIsOpen(false)} className="block rounded-xl px-4 py-3 text-base font-medium text-slate-700 transition-colors hover:bg-slate-100 hover:text-[#FFC916] dark:text-gray-300 dark:hover:bg-white/5">{isAdmin ? "Dashboard" : "Profile"}</Link>{!me.emailVerified && <Link href="/dashboard/settings" onClick={() => setIsOpen(false)} className="block rounded-xl px-4 py-3 text-base font-medium text-amber-600 transition-colors hover:bg-amber-50 hover:text-amber-700 dark:text-amber-300 dark:hover:bg-amber-500/10 dark:hover:text-amber-200">Verifikasi Email</Link>}<button onClick={handleLogout} className="block w-full rounded-xl px-4 py-3 text-left text-base font-medium text-red-600 transition-colors hover:bg-red-50 hover:text-red-700 dark:text-red-300 dark:hover:bg-red-500/10 dark:hover:text-red-200">Logout</button></div>}</div></motion.div>}</AnimatePresence>
        </nav>
    );
}
